...
//	fflush(msg_prot);
    ...
    std::unique_ptr<Message> recvMsg;
...
obj_conn::~obj_conn()
{
    ...
    auto &interfaces = mod->connectivity().interfaces;
    auto it = std::find_if(interfaces.begin(), interfaces.end(), [&s](const C_interface &inter) {
        return inter.get_name() == s;
    });
...
void object::del_to_connection(const string &name, const string &nr, const string &host, const string &intf)
{
    ...
    auto objConn = std::find_if(to.begin(), to.end(), [&name, &nr, &host, &intf](const obj_conn &conn) {
        return conn.get_mod()->info().name == name &&
               ...
               conn.get_mod()->host.userInfo().hostName == host &&
...
void object::newDataObject()
{
    ...
    std::for_each(to.begin(), to.end(), std::bind(&obj_conn::new_data, std::placeholders::_1, new_name));
}

void object::del_old_DO()
{
    auto &crb = dynamic_cast<const controller::CRBModule &>(get_from().get_mod()->host.getModule(sender_type::CRB));
    dataobj->reset();
    data *tmp_data;
...
void object::del_all_DO(int already_dead)
{
    ...
    const controller::CRBModule &crb = dynamic_cast<const controller::CRBModule &>(tmp_mod->host.getModule(sender_type::CRB));
    dataobj->reset();
    data *tmp_data;
    while ((tmp_data = dataobj->next()) != NULL)
    {
...
std::string remoteHostOrLOCAL(const std::string &localHostName, const std::string &localUserName, const controller::RemoteHost &host)
{
    if (host.userInfo().hostName == localHostName && host.userInfo().userName == localUserName)
    {
        return "LOCAL";
    }
    return host.userInfo().hostName;
}
...
string object::get_connection(int *i, const string &local_name, const string &local_user)
{
    ...
    if (from.get_mod() && from.get_mod()->isOriginal())
    {
        ...
        for (const obj_conn &conn : to)
        {
            ...
            {
                (*i)++;
                retval << from_str.str() << conn.get_mod()->info().name << "\n"
