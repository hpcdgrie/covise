...
DemoServer::DemoServer(const std::string &demosPath, int port)
    : m_port(port)
{
    m_logPath = (fs::path(demosPath) / "templates/launch_log.jsonl").string();
    m_demosJson = (fs::path(demosPath) / "templates/demos.json").string();
    m_staticDir = (fs::path(demosPath)).string() + "/";
    m_indexHtml = (fs::path(demosPath) / "templates/index.html").string();
    m_apps = {
        {"covise", "covise"},
        {"OpenCOVER", "OpenCOVER"},
        {"opencover", "OpenCOVER"},
...
void DemoServer::setupRoutes(crow::SimpleApp &app)
{
    ...
    CROW_ROUTE(app, "/launch_demo").methods("POST"_method)([this](const crow::request &req)
        ...
        // Log launch event
        std::ofstream logf(m_logPath, std::ios::app);
