...
private:
    struct Section
    {
        ...
        Section(size_t vertsPerCircle, double pointSize, const opencover::ColorMap& map, float min, float max, osg::MatrixTransform* parent);
        ~Section();
    ...
    void clear() override;
    void applyShader(const opencover::ColorMap& map, float min, float max) override;
