...
const string &uif::get_host() const
{
    return host;
}
...
void uiflist::create_uifs(const NetModule& app, const string &execname, const string &key)
{
    ...
    for (const auto &hostIt : app.host.hostManager)
    {
        ...
        new_uif->set_host(host->userInfo().hostName);
        new_uif->set_userid(host->userInfo().userName);

        auto ui =  host->getModule(sender_type::USERINTERFACE).as<Userinterface>();
        new_uif->set_status(ui->status());
        auto crb = host->getModule(sender_type::CRB).as<CRBModule>();
        this->add(new_uif);
        count++;
...
void modui::set_new_status()
{
    ...
    while ((tmp_uif = uif_list->next()) != NULL)
    {
        ...
        tmp_uif->set_status(app->host.getModule(sender_type::USERINTERFACE).as<Userinterface>()->status());
    ...
    while (1)
    {
        ...
        if (tmp->get_application()->info().name == name &&
         tmp->get_application()->instance() == std::stoi(nr) &&
          tmp->get_application()->host.userInfo().hostName == host)
            break;
    }
    return tmp;
}
