...
void VrmlNodeIFaceSet::initFields(VrmlNodeIFaceSet *node, VrmlNodeType *t)
{
    ...
    {
        initFieldsHelper(node, t,
                         exposedField("texCoord" + std::to_string(i), node->d_texCoords[i]),
                         field("texCoordIndex" + std::to_string(i), node->d_texCoordIndices[i]));
    }
    ...
    if(t)
    {
        for (size_t i = 0; i < MAX_TEXCOORDS; i++)
        {
            t->addEventIn(("set_texCoordIndex" + std::to_string(i)).c_str(), VrmlField::MFINT32);
        }
