...
ToolChanger::ToolChanger(ui::Menu* menu, const ToolChangerFiles &files,  osg::MatrixTransform *toolHead)
: m_toolHead(toolHead)
{
    ...
    m = osg::Matrix::scale(0.001, 0.001, 0.001) * m;
    ...
    m_changer = std::make_unique<Changer>(osgDB::readNodeFile(files.swapArm), m_trans); 
    m_changeDuration = m_changer->getAnimationDuration();

    m_trans->addChild(osgDB::readNodeFile(files.cover)); 
